...
import { fetchStudentMarksApi, fetchSubjectsApi } from '../../../api/academics';
import { MarksSkeleton } from './skeletons';
...
export const StudentMarksView: React.FC = () => {
  ...
  const subjectName = (id: string) => subjects.find((s) => s.id === id)?.name || `Subject ${id.slice(-4)}`;

  const load = useCallback(async () => {
    setError(null);
    try {
      const [marksRes, subjectsRes] = await Promise.all([
        fetchStudentMarksApi(),
  ...
  if (loading && !refreshing) {
    return (
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <ShimmerProvider><MarksSkeleton /></ShimmerProvider>
      </ScrollView>
    );
  }
      ...
      <View style={styles.heroBlock}>
        <View style={styles.heroHeader}>
          ...
          <View>
            ...
            <Text style={styles.heroSub}>Your exam results across subjects</Text>
          </View>
        </View>
                ...
                <View
                  style={[
                    styles.progressFill,
                    {
                      ...
                      backgroundColor: passed ? '#16A34A' : '#DC3545',
                    },
                  ]}
                />
              </View>
...
const styles = StyleSheet.create({
  ...
  errorTitle: { fontSize: 15, fontWeight: '800', color: '#1A1B1C' },
  errorSub: { fontSize: 12, color: '#6B6B6B', textAlign: 'center' },
  ...
  heroSub: { fontSize: 11, color: '#8A8A8A', marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.6, fontWeight: '700' },
  ...
  examName: { fontSize: 15, fontWeight: '800', color: '#1A1B1C' },
  examSub: { fontSize: 11, color: '#6B6B6B', marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '700' },
  ...
  subjectName: { fontSize: 13, fontWeight: '700', color: '#1A1B1C' },
  subjectScore: { flexDirection: 'row', alignItems: 'baseline', gap: 4 },
  scoreNum: { fontSize: 14, fontWeight: '800' },
  scoreMax: { fontSize: 11, color: '#6B6B6B', fontWeight: '700' },
  scorePct: { fontSize: 10, color: '#6B6B6B', fontWeight: '700', marginLeft: 6 },

  totalsRow: { flexDirection: 'row', justifyContent: 'space-between', paddingTop: 6, borderTopWidth: 1, borderTopColor: '#F3F1EA' },
  totalsLabel: { fontSize: 11, fontWeight: '700', color: '#6B6B6B', textTransform: 'uppercase', letterSpacing: 0.5 },
  totalsValue: { fontSize: 13, fontWeight: '800', color: '#1A1B1C' },
});
